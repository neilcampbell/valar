...
from smart_contracts.artifacts.noticeboard.client import EarningsDistribution
...
def test_action(
    ...
    # There is no change in delegator user
    ...
    # There is no change in validator user
...
def test_wrong_indices(
    ...
    # Action fail
    gs_del = noticeboard.get_delegator_global_state(del_app_id)
    with pytest.raises(LogicError) as e:
        action_inputs.val_owner = gs_del.del_manager
        action_inputs.val_app_idx = del_app_id
        noticeboard.delegator_action(
            app_id=del_app_id,
            action_name=TEST_ACTION_NAME,
            action_inputs=action_inputs,
    ...
    assert is_expected_logic_error(ERROR_USER_UNEXPECTED_ROLE, e)
