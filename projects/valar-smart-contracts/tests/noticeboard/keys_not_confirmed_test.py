...
def test_action(
    ...
    res = noticeboard.delegator_action(
        ...
        action_inputs=action_inputs,
        val_app=val_app_id,
        action_account=noticeboard.dispenser,  # A 3rd party reports the keys haven't been confirmed
    )
    ...
    assert base64.b64decode(res.tx_info["inner-txns"][-1]["txn"]["txn"]["note"]) == MSG_CORE_KEYS_NOT_CONFIRMED
    ...
    returned_amount = calc_operational_fee(
    ...
    gs_del_exp.state = dc.STATE_ENDED_NOT_CONFIRMED
...
def test_action_w_partner(
    ...
    # Partner should have earned only the setup fee portion
