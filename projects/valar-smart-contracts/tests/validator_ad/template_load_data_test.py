...
def test_action(
    ...
    action_inputs.template_test_data_chunk = bytes([0xAA] * 2048) + bytes([0xFF] * 2048)
    ...
    assert validator_ad.app_box(BOX_DELEGATOR_CONTRACT_TEMPLATE_KEY) == [
        ...
        action_inputs.template_test_data_chunk + \
        bytes([0x00] * (
            action_inputs.template_size - \
            action_inputs.template_test_offset - \
            len(action_inputs.template_test_data_chunk)
        )),
        True,
    ]
...
def test_state(
    ...
    if init_state == "TEMPLATE_LOAD":
        ...
        # Check new state of ValidatorAd - should remain in STATE_TEMPLATE_LOAD
