...
import { algoBigIntToDisplay, assetAmountToFeeDisplay, canStake, decodeValAdState, roundsToDate, roundsToDuration } from "@/utils/convert";
...
export async function getStakeList(
  ...
  const stakeListItems = Promise.all([...valAdsMap.values()]
  .map(async (gsValAd) =>{
