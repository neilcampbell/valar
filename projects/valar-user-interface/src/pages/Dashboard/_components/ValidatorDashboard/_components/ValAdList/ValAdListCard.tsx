...
const ValAdListCard = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      if (user) {
        ...
        const res = await fetchValOwnerAds(algorandClient.client.algod, user?.userInfo!);
  ...
  useEffect(() => {
    const fetch = async () => {
      if(valAdMap){
        const valAdList = await getValAdListItems(algorandClient.client.algod, valAdMap);
  ...
  const columns = [
    ...
    columnHelper.display({
      header: () => null,
      id: "details",
      cell: ({ row }) => <ValAdDrawer valAppId={row.original.adId} />,
      enableHiding: false,
    }),
  ];
