...
import { algoBigIntToDisplay, assetAmountToFeeDisplay, decodeValAdState, gratisBigIntToNumber, roundsToDate, roundsToDuration } from "@/utils/convert";
...
export async function getValAdListItems(
  algodClient: AlgodClient,
  valAdMap: Map<bigint, ValidatorAdGlobalState>,
): Promise<ValAdListItem[]> {

  const valAdList = await Promise.all([...valAdMap.values()]
  .map(async (gsValAd) => {
    const adId = gsValAd.appId;
    const status = decodeValAdState(gsValAd.state);
    ...
    const gated = gsValAd.termsReqs.gatingAsaList[0][0] !== 0n || gsValAd.termsReqs.gatingAsaList[1][0] !== 0n ? "Yes" : "No";
    ...
    return ({
      ...
      earnTotal: assetAmountToFeeDisplay(earnTotal, assetId, AssetParams.total),
      earnClaim: assetAmountToFeeDisplay(earnClaim, assetId, AssetParams.total),
      currency: currency,
      setupFee: assetAmountToFeeDisplay(gsValAd.termsPrice.feeSetup, assetId, AssetParams.setup),
      opFeeMin: assetAmountToFeeDisplay(gsValAd.termsPrice.feeRoundMin, assetId, AssetParams.opMin),
      opFeeVar: assetAmountToFeeDisplay(gsValAd.termsPrice.feeRoundVar, assetId, AssetParams.opVar),
      ...
      warningTime: roundsToDuration(gsValAd.termsWarn.roundsWarning, TimeParams.warn, true),
      gated: gated,
    }) as ValAdListItem;
  }));
